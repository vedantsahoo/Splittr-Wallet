...
function checkLimits(userId, amount, date, currency) {
    ...
    const user = db.prepare('SELECT daily_limit, monthly_limit FROM users WHERE id = ?').get(userId);
    if (!user)
        return;
...
router.get('/transactions', requireAuth, (req, res) => {
    try {
        const transactions = db.prepare('SELECT * FROM transactions WHERE user_id = ? ORDER BY date DESC, id DESC').all(req.userId);
...
router.post('/add-funds', requireAuth, (req, res) => {
    const { currency } = req.body;
    const userId = req.userId;
    const updateBalance = db.transaction(() => {
        const amount = parsePositiveAmount(req.body.amount);
        // 1. Update wallet balance
        const current = db.prepare('SELECT amount FROM wallet_balances WHERE user_id = ? AND currency = ?').get(userId, currency);
        if (!current) {
...
router.post('/send-money', requireAuth, (req, res) => {
    ...
    const performSend = db.transaction(() => {
        ...
        const currentSenderBal = db.prepare('SELECT amount FROM wallet_balances WHERE user_id = ? AND currency = ?').get(userId, currency);
        if (!currentSenderBal) {
            throw new Error(`Currency ${currency} not supported`);
        }
        ...
        const senderInitials = sender.name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2);
        const recipientInitials = cleanRecipientName.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2);
        if (receiver) {
            // 4. Update receiver's wallet balance
            const currentReceiverBal = db.prepare('SELECT amount FROM wallet_balances WHERE user_id = ? AND currency = ?').get(receiver.id, currency);
      ...
      `).run(`${Date.now()}-rec`, receiver.id, amount, currency, `Received from ${sender.name}`, txDate, sender.name, senderInitials);
        }
    ...
    `).run(Date.now().toString(), userId, amount, currency, `To ${cleanRecipientName}`, txDate, cleanRecipientName, recipientInitials);
...
router.get('/savings-goals', requireAuth, (req, res) => {
    try {
...
router.post('/savings-goals', requireAuth, (req, res) => {
    try {
...
router.put('/savings-goals/:id/progress', requireAuth, (req, res) => {
    const { id } = req.params;
    const userId = req.userId;
    const updateGoal = db.transaction(() => {
        ...
        const currentBal = db.prepare('SELECT amount FROM wallet_balances WHERE user_id = ? AND currency = ?').get(userId, goal.currency);
        if (currentBal) {
            db.prepare('UPDATE wallet_balances SET amount = ? WHERE user_id = ? AND currency = ?')
